...
def validate_move(
    ...
    if not is_valid_match:
        return {
            'success': False,
            'msg': f"First card must be a ({active_suit}) or value ({active_val})."
        }
    ...
    for card_idx, card in enumerate(matched_cards):
        if card_idx > 0:
            ...
            is_chain_valid = (
                ...
                card['value'] == 'Ace' or
                (is_dump_active and card['suit'] == dump_suit)
            )
            if not is_chain_valid:
                return {
                    'success': False,
                    'msg': "Chain invalid: Cards must match rank, suit, "
                           "be an Ace, or follow a Queen"
                }
        if card['value'] == '8':
            eight_skips += 1
        temp_penalty_type, temp_accumulated, temp_source = calculate_penalty_update(
            card, temp_penalty_type, temp_accumulated, player_name, temp_source
        )

    return {
        'success': True, 'msg': "Success", 'eight_skips': eight_skips,
        'new_penalty_type': temp_penalty_type,
        'new_accumulated_penalty': temp_accumulated,
        'new_penalty_source': temp_source
    }
